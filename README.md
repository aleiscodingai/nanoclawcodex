...
Using Codex CLI, NanoClaw can dynamically rewrite its code to customize its feature set for your needs.

**New:** First AI assistant to support agent swarms. Spin up teams of agents that collaborate in your chat. Learn more at https://developers.openai.com/codex
...
**Best harness, best model.** NanoClaw runs on Codex CLI, which means you're running Codex directly. Codex is highly capable and its coding and problem-solving capabilities allow it to modify and expand NanoClaw and tailor it to each user.

## What It Supports

- **Messenger I/O** - Message NanoClaw from your phone. Supports WhatsApp, Telegram, Discord, Slack, Signal and headless operation.
- **Isolated group context** - Each group has its own `CODEX.md` memory, isolated filesystem, and runs in its own container sandbox with only that filesystem mounted to it.
...
If you want to add Telegram support, don't create a PR that adds Telegram alongside WhatsApp. Instead, contribute a skill file (`.codex/skills/add-telegram/SKILL.md`) that teaches Codex how to transform a NanoClaw installation to use Telegram.
...
- `groups/*/CODEX.md` - Per-group memory
