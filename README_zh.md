...
**新功能：** 首个支持 [Agent Swarms（智能体集群）](https://developers.openai.com/codex) 的 AI 助手。可轻松组建智能体团队，在您的聊天中高效协作。
...
然后运行 `/setup`。Codex CLI 会处理一切：依赖安装、身份验证、容器设置、服务配置。
...
**AI 原生:** 无安装向导(由 Codex CLI 指导安装)。无需监控仪表盘，直接询问 Codex 即可了解系统状况。无调试工具(描述问题，Codex 会修复它)。

**技能（Skills）优于功能（Features）:** 贡献者不应该向代码库添加新功能（例如支持 Telegram）。相反，他们应该贡献像 `/add-telegram` 这样的 [Codex CLI 技能](https://developers.openai.com/codex)，这些技能可以改造您的 fork。最终，您得到的是只做您需要事情的整洁代码。

**最好的工具套件，最好的模型:** 本项目运行在 Codex CLI 之上，这意味着您直接运行的就是 Codex CLI。工具套件至关重要。一个低效的工具套件会让再聪明的模型也显得迟钝，而一个优秀的套件则能赋予它们超凡的能力。Codex CLI (在我看来) 是市面上最好的工具套件。

## 功能支持

- **WhatsApp 输入/输出** - 通过手机给 Codex 发消息
- **隔离的群组上下文** - 每个群组都拥有独立的 `CODEX.md` 记忆和隔离的文件系统。它们在各自的容器沙箱中运行，且仅挂载所需的文件系统。
- **主频道** - 您的私有频道（self-chat），用于管理控制；其他所有群组都完全隔离
- **计划任务** - 运行 Codex 的周期性作业，并可以给您回发消息
- **网络访问** - 搜索和抓取网页内容
...
**会话管理**
- `/add-clear` - 添加一个 `/clear` 命令，用于压缩会话（在同一会话中总结上下文，同时保留关键信息）。这需要研究如何通过 Codex CLI 以编程方式触发压缩。
...
我不知道。运行 `codex`，然后运行 `/debug`。如果 Codex 发现一个可能影响其他用户的问题，请开一个 PR 来修改 `SKILL.md` 安装文件。
