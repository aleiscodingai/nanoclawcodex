...
│  │    • CODEX.md → /workspace/global/CODEX.md (non-main ro)        │   │
...
| Agent | Codex CLI | Run Codex with tools and MCP servers |
...
```
nanoclaw/
├── CODEX.md                      # Global memory + project context for Codex CLI
├── docs/
...
│
├── container/
│   ├── Dockerfile                 # Container image (runs as 'node' user, includes Codex CLI CLI)
...
│   │   ├── CODEX.md              # Main channel memory
│   │   └── logs/                  # Task execution logs
│   └── {Group Name}/              # Per-group folders (created on registration)
│       ├── CODEX.md              # Group-specific memory
...
Authenticate with the Codex CLI using device auth:

```bash
codex login --device-auth
```

Credentials are stored under `~/.codex` and copied into each group's `.codex/` directory before container startup.
...
NanoClaw uses a hierarchical memory system based on CODEX.md files.

### Memory Hierarchy

| Level | Location | Read By | Written By | Purpose |
|-------|----------|---------|------------|---------|
| **Global** | `CODEX.md` | All groups | Main only | Preferences, facts, context shared across all conversations |
| **Group** | `groups/{name}/CODEX.md` | That group | That group | Group-specific context, conversation memory |
...
1. **Agent Context Loading**
   ...
   - The agent runner prepends both:
     - `/workspace/global/CODEX.md` (global memory)
     - `/workspace/group/CODEX.md` (group memory)
     to the user prompt before calling Codex CLI.

2. **Writing Memory**
   - When user says "remember this", agent writes to `./CODEX.md`
   - When user says "remember this globally" (main channel only), agent writes to `/workspace/global/CODEX.md`
...
8. Codex processes message:
   ├── Reads CODEX.md files for context
...
- Codex's built-in safety training
...
| "Codex CLI process exited with code 1" | Container runtime failed to start | Check logs; NanoClaw auto-starts container runtime but may fail |
